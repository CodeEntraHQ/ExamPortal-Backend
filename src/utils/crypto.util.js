...
const getJwtPayload = (user_id, type) => {
...
const getUserInvitationLink = (user_id) => {
  const payload = getJwtPayload(user_id, TOKEN_TYPES.USER_INVITATION);
...
const getResetPasswordLink = (user_id) => {
  const payload = getJwtPayload(user_id, TOKEN_TYPES.RESET_PASSWORD);
...
const generateUserSessionToken = (user_id) => {
  const payload = getJwtPayload(user_id, TOKEN_TYPES.USER_SESSION);
